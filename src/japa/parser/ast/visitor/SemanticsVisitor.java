...
public final class SemanticsVisitor implements VoidVisitor<Object> {
    ...
    public void visit(FieldDeclaration n, Object arg) {
        ...
        Type type = n.getType();
        Symbol symOfFieldType = currentScope.resolve(type.toString());
        
        // Checks the type of the field
        checkType(type, symOfFieldType);
        ...
        for (Iterator<VariableDeclarator> i = n.getVariables().iterator(); i.hasNext();) {
            ...
            checkName(type, symOfFieldType, var.getId());
        }
    ...
    public void visit(VariableDeclarationExpr n, Object arg) {
		...
		Type type = n.getType();
		Symbol symOfVariableType = currentScope.resolve(type.toString());

		// Checks the type of the variable
		checkType(type, symOfVariableType);
		...
		if(expression!=null){
			...
			if(expression.getClass() == NameExpr.class){
				...
				if(!(sym.getType() instanceof symtab.Type)){
					throw new A2SemanticsException(expression + " on line " + expression.getBeginLine() + " is not valid");
				}
	...
	/**
	 * Checks if the type of a variable (including field) is valid
	 * @param type Type of the variable or expression
	 * @param symOfVariableType Symbol of the type
	 */
	private void checkType(Node type, Symbol symOfVariableType) {
		if (symOfVariableType == null) {
			throw new A2SemanticsException(type + " on line " + type.getBeginLine() + " is not a defined type");
		}

		if (!(symOfVariableType instanceof symtab.Type)) {
			//System.out.println(symOfVariableType.getClass());
			throw new A2SemanticsException(type + " on line " + type.getBeginLine() + " is not a valid type");
		}
	}
